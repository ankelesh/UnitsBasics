...
namespace Model
{
	...
	const damagePointer PI_BI_6(
		new LightMachWeapons(
			standard_infantry_weapon_charset({ L"BI-6", 25, false, 2, 0, 1, 4 }), 5
		));
	...
	const damagePointer RE_SMG_14mm(
		new SubmachWeapons(
			standard_infantry_weapon_charset({ L"SMG 14mm", 30, false, 10, 0, 1, 1 })
		));
	...
	const damagePointer RE_SMG_16mm(
		new SubmachWeapons(
			standard_infantry_weapon_charset({ L"SMG 16mm", 30, false, 15, 0, 1, 1 })
		));
		// Рокский 16мм ПП используется для штурмов
	const damagePointer RE_SLR_25mm(
		new RockInfantryWeapons(
			standard_infantry_weapon_charset({ L"SLR 25mm", 70, false, 40, 0, 1, 7 })
		));
	...
	const damagePointer RE_AR_20mm(
		new RockInfantryWeapons(
			standard_infantry_weapon_charset({ L"AR 20mm", 70, false, 30, 0, 1, 8 })
		));
	...
	const damagePointer RE_SR_35mm(
		new RockSniperWeapons(
			standard_infantry_weapon_charset({ L"SR 35mm", 35, false, 60, 0, 1, 9 })
			, 1, 4));
	...
	const damagePointer CR_MU_SB(
		new InfantryWeapons(
			standard_infantry_weapon_charset({ L"Musket", 40, false, 1, 0, 1, 3 })
		));
	...
	const damagePointer CR_MU_TH(
		new InfantryWeapons(
			standard_infantry_weapon_charset({ L"Musket", 40, false, 1, 0, 1, 5 })
		));
		// Нарезные винтовки состоят на вооружении ветеранской пехоты криогеников.
	const damagePointer CR_NP(
		new InfantryWeapons(
			standard_infantry_weapon_charset({ L"Pistol", 15, false, 1, 0, 1, 1 })
		));
